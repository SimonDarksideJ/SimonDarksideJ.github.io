---
layout: post
title: DotNet 8, meet MonoGame
date: 2023-12-11 00:00 +0000
description: The journey has begun and the road to building DotNet 8 based games has begun.
...
![Get the DotNet8 Party started](/assets/img/posts/20231211/getthepartystarted.gif)

> TL;DR -> DotNet 8 language support is now available for MonoGame and opening up a whole new world of goodness and speed for games.
...
* [What will DotNet 8 do for MonoGame](#what-will-dotnet-8-do-for-monogame)
* [Actually, you can build DotNet 8 already](#actually-you-can-build-dotnet-8-already)
* [Accessing the Developer version of the MonoGame packages](#accessing-the-developer-version-of-the-monogame-packages)
* [Getting access to the DotNet 8 developer version of MonoGame with Visual Studio](#getting-access-to-the-dotnet-8-developer-version-of-monogame-with-visual-studio)
* [Getting access to the DotNet 8 developer version of MonoGame with Visual Studio Code](#getting-access-to-the-dotnet-8-developer-version-of-monogame-with-visual-studio-code)
* [Do not forget about the tools!](#do-not-forget-about-the-tools)

## What will DotNet 8 do for MonoGame

Initially, not much.  Apart from inheriting DotNet 8's inherent speed upgrades and compilation updates, nothing has really changed.  Stuff you get for free for being on the latest version of DotNet is just that, free stuff.

What this enables however, is for MonoGame to start utilizing some of the additional features enabled with the latest and greatest DotNet framework, namely:
...
While it will take time to fully realize some of these benefits in the core part of the MonoGame library, where it will initially provide a lot of advantages in the Content Pipeline and the writing of some really cool things with Content Pipeline extensions, which are a fantastic way to fully empower your content in any MonoGame project.

But you are now free to fully utilize any and all improvements from the DotNet 8 SDK without limitation (other than making sure it works on your intended platform).

> One small Caveat to the announcement is that the DotNet upgrade is ONLY for the **public** version of MonoGame. For consoles and other private areas (due to the licensing enforced by partners), the team is working hard in this release to get those updated, but that will come later.

## Actually, you can build DotNet 8 already

Now, one misnomer to get over at this point, is that you can **TODAY**, with the **release version** of MonoGame, actually build a DotNet 8 executable for your game, and in that, use DotNet 8 features in your project.  But the MonoGame Libraries are still DotNet 6 and limited to the DotNet 6 instruction set.  Like building a super fast car but only putting the engine for a Scooter in it.

You still get some of the performance gains in your DotNet project for the code you write in your game, but it is not the full gain from an entirely DotNet 8 project.
...
You can download each package from here manually if you wish, but it is better to do it directly in your package, to do this however, you will also need a **Personal Access Token** for your client to successfully authenticate with GitHub to access the packages.
...
6. As shown above, give the token a recognizable name, select the **read:packages** scope (which only allows this token to read packages, nothing else) and finally set the expiration date.  (For read tokens like this, I usually set the expiration to "Never expire", although GitHub will warn you if you do)
...
## Getting access to the DotNet 8 developer version of MonoGame with Visual Studio

![Visual Studio Logo](/assets/img/posts/20231211/VisualStudio2022.jpg)

Starting with Visual Studio, as it is the simplest to do, upgrading MonoGame to DotNet 8 is as simple as adding access to the GitHub NuGet package source and updating your packages.
...
From here you should simply be able to select the installed packages and update them to the latest without issue.  Congrats you are now using the DotNet 8 version of the MonoGame Framework!

Next Step, [jump to this section](#do-not-forget-about-the-tools) to also update the MonoGame tools, e.g. the MGCB content tool.

## Getting access to the DotNet 8 developer version of MonoGame with Visual Studio Code

![Visual Studio Code Logo](/assets/img/posts/20231211/visualstudiocode.png)

For Visual Studio Code, the flow is a little more manual and also a bit trickier as we no longer have a UI in which to make changes we need to apply the updates by hand.

1. Open Visual Studio Code in your MonoGame Project folder.
2. Select the **CSPROJ** file in the folder tree.
3. Change the **TargetFramework** value from **net6.0** to **net8.0**. (yay your project will now build for DotNet 8)

    > ```<TargetFramework>net8.0</TargetFramework>```

4. Next, to be able to access the GitHub Packages, we need to create an Authentication configuration file. In the root of your project, Create a new "**nuget.config**" text file and replace its contents with the following:
    ...
    <configuration>
        <packageSources>
            <clear />
            <add key="monogame development" value="https://nuget.pkg.github.com/MONOGAME/index.json" />
        </packageSources>
        <packageSourceCredentials>
            <github>
                <add key="Username" value="YOUR GitHub Username or email" />
...
5. With the authentication file in place, open a new **Terminal Window** in VSCode ("**Terminal -> New Terminal**" or "**Ctrl+Shift+'**")
6. Enter the following command for each package you have installed:

    ```dotnet add package MonoGame.Content.Builder.Task --version 3.8.1.534-develop```
...
You have updated your project to DotNet 8 as well as your MonoGame Framework dependencies.  So, why when you do a build does it still use the old DotNet 6 versions of the MonoGame Tools (like MGCB)?

The simple answer is because you are still telling it to.

To update your project to use the newer DotNet 8 version of the tools, you also need to update the **dotnet-tools.json** configuration located in your projects **.config** folder.
...
Done, next time you perform a ```dotnet restore``` or build your project, the latest version of the tools with be downloaded and made available (they will not be available until you do your FIRST build after making the update)

> **Note* this is still a manual task. Every time you update the dependencies, you will also need to update the tools versions.

## Conclusion

To the beginner, this upgrade will not mean much at all, except to state that you will be running on the latest "supported" version of Dot Net (as DotNet 6 is now out of support), which also means you will get all the latest fixes and updates to the backbone of your project. As well as the modest speed improvements from the latest and greatest of DotNet.

For the more adventurous, newer C# features and DotNet 8 specific functionality are now within your project, so much so that I couldn't fit them into a single article :D

> ### [But you can check out the DotNet 8 docs for even more details.](https://learn.microsoft.com/en-us/dotnet/core/whats-new/dotnet-8)
